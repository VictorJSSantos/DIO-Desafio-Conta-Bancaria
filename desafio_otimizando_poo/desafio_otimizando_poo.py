...
if __name__ == "__main__":
    ...
    """
        ...
        if resposta.lower() == "g":
            if cliente:
                if cliente.contas:
                    # Caso o cliente tenha mais de uma conta, pergunte qual conta deseja mexer
                    if len(cliente.contas) > 1:
                        print("Você possui as seguintes contas:")
                        for i, conta in enumerate(cliente.contas, start=1):
                            print(f"{i} - {conta}")
                        conta_escolhida = (
                            int(input("Digite o número da conta que deseja usar: ")) - 1
                        )
                        conta = cliente.contas[conta_escolhida]
                    else:
                        conta = cliente.contas[
                            0
                        ]  # Se o cliente tiver apenas uma conta, usa a primeira conta

                    valor = float(input("Digite o valor a ser sacado: "))
                    conta.sacar(valor)
                else:
                    print("Você não possui contas para realizar o saque.")
                    input("Aperte qualquer tecla para continuar")
            else:
        ...
        if resposta.lower() == "h":
            if cliente:
                if cliente.contas:
                    ...
                    print("Você não possui contas para realizar o depósito.")
                    input("Aperte qualquer tecla para continuar")
            else:
                print("Cliente não encontrado.")
                input("Aperte qualquer tecla para continuar")
        ...
        if resposta.lower() == "i":
            if cliente:
                if cliente.contas:
                    ...
                    print("Você não possui contas para visualizar o extrato.")
                    input("Aperte qualquer tecla para continuar")
            else:
                print("Cliente não encontrado.")
                input("Aperte qualquer tecla para continuar")
